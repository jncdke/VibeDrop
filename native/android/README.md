...
7. App 运行后会启动 `ClipboardSyncService`，从 Room 读取已保存 Mac，建立后台 WebSocket，收到 `clipboard` 消息后写入 Android 剪贴板；后台直接写入失败时会启动 1x1 透明 Activity 兜底。设置页会显示本机身份、网络能力、VPN 状态、局域网地址、已保存 Mac 状态和最近连接错误。

...
9. 发送页“传图到剪贴板”使用 Android 图片选择器读取图片并发送 `image_clipboard`；“传到收件箱”使用系统文件选择器读取单个或多个 URI，并按 192KiB 分片发送 `incoming_file_start/chunk/complete`，发送时监控 OkHttp WebSocket 队列做背压，并等待 Mac 返回 `incoming_file_saved` 后才写成功历史。

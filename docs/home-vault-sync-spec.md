...
同步报告需要至少包含：源历史文件行数、解析成功数、解析失败数、去重后历史条数、媒体引用数、成功入库媒体数、原始文件缺失数、对象库新增/复用状态、Android 导入状态、远端路径和查看器 URL。查看器界面只把既没有对象文件也没有缩略图的媒体显示为缺失；有缩略图的记录直接展示缩略图，不额外提示原图状态。

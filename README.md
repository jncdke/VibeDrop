...
GitHub Release 不再走自动打包。当前发布方式改成：

1. 本地完成签名和真机验证
2. 本地生成 macOS 和 Android 构建产物
3. 手动把本地产物上传到 GitHub Release

推荐流程：

```bash
# 1. 本地构建并验证
./scripts/deploy-desktop.sh --skip-icons
./scripts/deploy-android.sh --skip-icons
...
# 4. 用本地产物创建 / 更新 GitHub Release
gh release create v0.1.4 \
  /path/to/VibeDrop_0.1.4_macos.zip \
  /path/to/VibeDrop_0.1.4_android.apk
```

建议上传的本地产物：

- macOS：本地签名好的 `VibeDrop.app` 压缩包
- Android：本地签名好的 `VibeDrop-signed.apk`
- 可选：`.aab` 和 `SHA256SUMS`

这样做的原因很直接：本地构建出来的安装包已经在你的 Mac 和安卓真机上验证过，和你实际使用的是同一份，不会再出现 GitHub 远程打包出来能下但装不上的偏差。
...
2. **文件同步**：修改手机端前端代码后，**必须手动同步**到 Mac 端的 `static/` 目录，否则手机浏览器版不会更新。现在 CI workflow 仍然会自动校验这三份共享文件是否同步
